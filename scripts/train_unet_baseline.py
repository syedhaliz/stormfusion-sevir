"""
Stage 2: Baseline U-Net Training Script

Train UNet2D on tiny SEVIR split with MSE loss.
Target: CSI@74 > persistence baseline (~0.15)
"""
...
LOG_FILE = "outputs/logs/02_baseline_unet.log"
...
def main():
    ...
    print(f"\n{'='*80}")
    print("STAGE 2: BASELINE U-NET TRAINING")
    print(f"{'='*80}")
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"CSI@74:     {csi_74:.3f}  (target: >0.15)")
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint_path = os.path.join(CHECKPOINT_DIR, "unet_baseline_best.pt")
            torch.save({
                'epoch': epoch + 1,
    ...
    history_path = os.path.join(CHECKPOINT_DIR, "unet_baseline_history.json")
    ...
    with open(LOG_FILE, 'w') as f:
        f.write("="*80 + "\n")
        f.write("STAGE 2: BASELINE U-NET TRAINING LOG\n")
        f.write("="*80 + "\n\n")
